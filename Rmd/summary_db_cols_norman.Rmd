...
# schema = 'norman'
# table = 'data2_newest'
# cols = c('nor1', 'nor3', 'nor4', 'nor5', 'nor6')
```
...
var = read_query(user = DBuser, host = DBhost, port = DBport, password = DBpassword, dbname = DBetox,
                 query = q)
```
...
```{r write-single-file, message=FALSE}
# write summary to single file
dat = cbind_fill(dflist = l, limit = 5000)
# names(dat)[ seq(1, length(names(dat)), 2) + 1 ] = paste0(names(dat)[ seq(1, length(names(dat)), 2) ], '_n')

fwrite(dat, file.path(params$output_dir, paste0(params$table, '_summary.csv')))
```
