...
stx_filter = function(test = NULL,
  ...
  if (!is.null(concentration_unit_)) {
    test = test[ concentration_unit %in% tolower(concentration_unit_) ]
  }
  if (!is.null(concentration_type_)) {
    test = test[ concentration_type %in% tolower(concentration_type_) ]
  }
  if (!is.null(effect_)) {
    test = test[ effect %in% tolower(effect_)  ]
  }
  if (!is.null(endpoint_)) {
    test = test[ endpoint %in% endpoint_ ]
  }
  if (!is.null(exposure_)) {
    test = test[ exposure %in% tolower(exposure_) ]
  }
  ...
  if (!is.null(ecotox_grp_)) {
    taxa = taxa[ ecotox_grp %in% tolower(ecotox_grp_) ]
  }
  # merge -------------------------------------------------------------------
