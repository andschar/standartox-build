...
\begin{tabular}{cc}
    ...
    \midrule
    devtools: Tools to Make Developing R Packages Easier & \citep{wickham_devtools_2019} \\
    RCurl: General Network HTTP FTP Client Interface for R & \citep{lang_rcurl_2020} \\
    stringr: Simple Consistent Wrappers for Common String Operations & \citep{wickham_stringr_2019} \\
    R: Various Programming Utilities & \citep{bengtsson_r_2019} \\
    udunits: Udunits 2 Bindings for R & \citep{hiebert_udunits_2016} \\
    rvest: Easily Harvest Scrape Web Pages & \citep{wickham_rvest_2019} \\
    httr: Tools for Working with URLs and HTTP & \citep{wickham_httr_2019} \\
    The: A Practical and Consistent Mapping Between JSON Data and R Objects & \citep{ooms_the_2014} \\
    readxl: Read Excel Files & \citep{wickham_readxl_2019} \\
    openxlsx: Read Write and Edit xlsx Files & \citep{schauberger_openxlsx_2019} \\
    purrr: Functional Programming Tools & \citep{henry_purrr_2019} \\ data: Extension of data frame & \citep{dowle_data_2019} \\
    RPostgreSQL: R Interface to the PostgreSQL Database System & \citep{conway_rpostgresql_2017} \\ DBI: R Database Interface & \citep{b_dbi_2019} \\
    vegan: Community Ecology Package & \citep{oksanen_vegan_2019} \\ The: NA & \citep{wickham_the_2011} \\
    outliers: Tests for outliers & \citep{komsta_outliers_2011} \\ feather: R Bindings to the Feather API & \citep{wickham_feather_2019} \\
    ggplot: Elegant Graphics for Data Analysis & \citep{wickham_ggplot_2016} \\ ggrepel: Automatically Position Non Overlapping Text Labels with & \citep{slowikowski_ggrepel_2019} \\
    ggridges: Ridgeline Plots in ggplot2 & \citep{wilke_ggridges_2018} \\
    scales: Scale Functions for Visualization & \citep{wickham_scales_2019} \\ treemapify: Draw Treemaps in ggplot2 & \citep{wilkins_treemapify_2019} \\ cowplot: Streamlined Plot Theme and Plot Annotations for ggplot2 & \citep{wilke_cowplot_2019} \\
    RColorBrewer: ColorBrewer Palettes & \citep{neuwirth_rcolorbrewer_2014} \\ treemap: Treemap Visualization & \citep{tennekes_treemap_2017} \\ rgbif: Interface to the Global Biodiversity Information Facility API & \citep{chamberlain_rgbif_2020} \\
    taxize: NA & \citep{chamberlain_taxize_2013} \\ countrycode: An R package to convert country names and country codes & \citep{bundock_countrycode_2018} \\ foreach: Provides Foreach Looping Construct for R & \citep{microsoft_foreach_2017} \\
    doParallel: Foreach Parallel Adaptor for the parallel Package & \citep{corporation_doparallel_2018} \\
    fst: Lightning Fast Serialization of Data Frames for R & \citep{klik_fst_2019} \\
    DT: A Wrapper of the JavaScript Library DataTables & \citep{xie_dt_2019} \\
    knitr: A General Purpose Package for Dynamic Report Generation in R & \citep{xie_knitr_2020} \\
    \bottomrule
\end{tabular}
\end{table}
...
\bibliography{refs/references-standartox.bib,
              refs/references-standartox-build.bib}
