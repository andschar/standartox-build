...
\tikzset{
	...
	data/.style = {rectangle, minimum height = 3cm, minimum width = 7cm, rounded corners, draw = black},
	app/.style = {circle, minimum height = 3cm, fill = red!40},
	api/.style = {circle, minimum height = 3cm, fill = red!40},
	rpackage/.style = {circle, minimum height = 3cm, fill = red!40},
...
\node (db-chemical) [db, right of = cas, xshift = 4cm] {Chemical\\data bases};
\node (db-habitat) [db, right of = taxa, xshift = 4cm, yshift = 1cm] {Habitat\\data bases};
\node (db-region) [db, right of = taxa, xshift = 4cm, yshift = -1cm] {Region\\data bases};
% standartox data
\node (stx-data) [data, below of = bg1-pipeline, xshift = 5.5cm, yshift = -6cm] {Standatox\\data set};
% user interfaces
\node (stx-app) [app, below of = stx-data, yshift = -3.5cm, xshift = -2cm] {Web\\application};
...
\begin{pgfonlayer}{bg2}
    \node (bg1-pipeline) [background, minimum height = 9cm, minimum width = 16cm, above of = epa-ecotox, xshift = 5.5cm, yshift = -1cm];
    \node (bg1-user) [background, below of = bg1-pipeline, minimum height = 7cm, minimum width = 12cm, yshift = -12cm];
\end{pgfonlayer}
%% arrows
% small
...
\begin{pgfonlayer}{bg2}
    ...
    \draw [arrow2] (stx-data.south) -- (stx-app.north);
    \draw [arrow2] (stx-data.south) -- (stx-api.north);
\end{pgfonlayer}
