...
\tikzset{
	...
	\node (chemscr) [main, right of=cas, xshift=4.5cm] {PHYS-CHEM}
    	...
    	child { node [children] { Chemspider - \textbf{qu\_chemspider\_scrape.R } }}
    	child { node [children] { Eurostat - \textbf{qu\_eurostat\_chem\_class.R} }}
    	child { node [children] { PubChem - \textbf{qu\_pc.R} }}
	...
	\node (habitat) [main, right of = taxon, xshift = 4.5cm, yshift = 1.5cm] {Habitats}
    	child { node [children] { WORMS data base - \textbf{qu\_worms2.R} }};
	...
	\node (checksres) [main, left of = analysesres, xshift = -4.5cm] {checks}
	    child { node [children]  { \textbf{re\_checks\_internal.R} }};
	...
	\node (writeapp) [main, right of = writedb, xshift=3cm, align = center] {Write to\\application}
	    child { node [children] { \textbf{wr\_app.R}  }};
	\node (writemeta) [main, right of = writeapp, xshift=3cm, align = center] {Write\\Meta data}
	    child { node [children] { \textbf{wr\_meta.R} }};
	\node (writestats) [main, right of = writemeta, xshift=3cm, align = center] {Write\\statistics}
