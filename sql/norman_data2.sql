...
/* Test substance */
  ...
  coalesce(cc.chem_name, 'not reported') "nor26", --Vehicle",
  CASE 
    WHEN cc.characteristics IN  ('NR', 'NC', '', ' ',  '--')
      THEN 'not reported'
    ELSE lower(coalesce(cc.characteristics, 'not reported'))
...
/* Biotest */
  ...
  coalesce(effect_lookup.description_norman, 'not reported') AS "nor35", --Effect
  coalesce(lower(measurement_codes.description), 'not reported') AS "nor36", --Effect measurement 
...
/* Test organism */
  ...
  lower(COALESCE(cm.control_mortality, 'not reported')) AS "nor51", --Mortality in the control",
  CASE 
...
----------------------------------------------
/* Dosing system */
  COALESCE(dd.do, 'not reported') AS "nor61", --Nominal concentrations",
  ...
  COALESCE(pm.mortality, 'not reported') AS "nor79", --Effect in positive control",
  COALESCE(dd.vc, 'not reported') AS "nor80", --Vehicle control",
  COALESCE(vm.vehicle_mortality, 'not reported') AS "nor81", --Effects in vehicle control",
----------------------------------------------
/* Test conditions */
...
/* Statistical design */
  ...
  CASE
    WHEN tests.experimental_design = ''
      THEN 'not reported'
    ELSE COALESCE(lower(tests.experimental_design), 'not reported') 
  END AS "nor116", --Number of replicates per concentration", 
  CASE
    WHEN results.significance_comments = ''
      THEN 'not reported'
  ELSE  COALESCE(lower(results.significance_comments), 'not reported') 
  ...
  END AS "nor118", --Trend",
  COALESCE(lower(statistical_significance_codes.description), 'not reported') AS "nor119", --Significance of result",
  CASE 
    WHEN results.significance_level_mean IN  ('NR', 'NC', '', ' ',  '--')
      THEN 'not reported'
    ELSE COALESCE(results.significance_level_mean, 'not reported') 
  ...
  */
  ...
  CASE 
    WHEN tests.other_effect_comments = '' 
      THEN 'not reported'
    ELSE coalesce(lower(tests.other_effect_comments), 'not reported') 
  END AS "nor126", --Other effects", 
  CASE
    WHEN results.additional_comments = ' ' AND tests.test_characteristics IN (' ', 'NR')
      THEN 'not reported'
    ELSE coalesce(lower(results.additional_comments), 'not reported')  || '; ' || tests.test_characteristics
  END AS "nor127", --Results comment",
  'n.a.'::text AS "nor128", --Test result plausible and consistent with other findings",
  coalesce(drm.drm_text, 'not reported') AS "nor129", --Dose-response reported in figure/text/table",
----------------------------------------------
/* Evaluation */
  CASE
  ...
  '5'::text AS "nor135", --Reliability score system used (5 = not yet assessed for quality)",
