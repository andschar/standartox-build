...
# Setup
```{r SETUP-familylookup, eval=TRUE}
## packages
require(data.table)
require(knitr)
...
lookup = rbindlist(list(lookup,
               ...
               as.list(c('Dicranaceae', NA, 'Plants', 'Bryopsida', 'class', '0', '0', '1'))
))
```

# Algae (monophyletic group) - Algen
...
# Euglenozoa (phylum)
There are autotrophic, heterotrophic species and parasites.
```{r euglenozoa}
lookup = rbindlist(list(lookup,
...
Heterotrophic. Rotifers eat particulate organic detritus. Mostly in freshwater environmemnts.
```{r rotifera-list}
lookup = rbindlist(list(lookup,
...
```{r fungi-list}
 # fungi (#!all terrestrial? not clear!!)
lookup = rbindlist(list(lookup,
...
lookup = rbindlist(list(lookup,
               ...
               as.list(c('Tipulidae', 'Ibisfliegen', 'Insecta', 'Diptera', 'order', '0', '0', '1')),
...
```{r todo}
todo_habitat =
  unique(tests2[ !family %in% lookup$family, ]$latin_BIname)
if (length(todo_habitat) == 0) {
...
```{r lookup-macrophytes, eval=FALSE}
...
# Write to file
```{r write-to-file-rm-overhead}
fwrite(lookup, file.path(tempdir(), 'family_lookup_list.csv'))
```
