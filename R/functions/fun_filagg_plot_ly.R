...
filagg_gg = function(dt,
  ...
  gg_out = ggplot(dt, aes(y = reorder(get(yaxis), -value),
    ...
    geom_point() +
    { if (xaxis == 'limout') coord_cartesian(xlim = range(dt$value)) } +
    { if (xaxis == 'log') scale_x_log10() } +
...
filagg_ly = function(dt,
  ...
  yform = list(categoryorder = "array",
               ...
               type = 'category',
               nticks = 1000) #! dummy to plot all y entries
  if (xaxis == 'log') {
  ...
  ly_out = plot_ly() %>%  
    add_trace(data = dt_all, x = ~value_fin, y = ~get(yaxis),
    ...
    add_trace(data = dt, x = ~value, y = ~get(yaxis),
...
filagg_pl = function(data,
  ...
  if (plot_type == 'dynamic') {
    pl_l = lapply(l[[1]], FUN = filagg_ly, dt_all = l[[2]], yaxis = yaxis, xaxis = xaxis)
    out = subplot(pl_l,
                  nrows = length(l[[1]]),
                  titleX = TRUE)
  }
  ...
  if (plot_type == 'static') {
    pl_l = lapply(l[[1]], FUN = filagg_gg, dt_all = l[[2]], yaxis = yaxis, xaxis = xaxis)
