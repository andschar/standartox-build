...
chebi_prop = Reduce(function(...) merge(..., by = 'chebiid', all = TRUE), l)
chebi_prop[ , mass := as.numeric(mass) ]
# names
clean_names(chebi_prop)
setcolorder(chebi_prop, c('cas', 'chebiid', 'chebiasciiname', 'iupac_name', 'formula'))
chebi_prop = chebi_prop[ !duplicated(cas) & !is.na(cas) ]
# classification ----------------------------------------------------------
## roles
chebi_role = copy(ont_par)
cols_role = c('acaricide', 'antibiotic', 'antifouling', 'avicide', 'biocide', 'ectoparasiticide',
              'drug', 'fungicide', 'herbicide', 'fumigant',
              'herbicide_safener', 'insecticide', 'molluscicide', 'nematicide', 'pediculicide', 
              'pesticide', 'pesticide_synergist', 'phytogenic', 'precursor', 'proacaricide',
              'profungicide', 'proherbicide', 
              'proinsecticide', 'pronematicide', 'rodenticide', 'scabicide', 'schistosomicide')
for (i in cols_role) {
  chebi_role[ grep(i, chebiName), (i) := TRUE ]
}
chebi_role_m = melt(chebi_role[ , .SD, .SDcols =! 'chebiName' ], id.vars = c('cas', 'chebiid'))
chebi_role2 = dcast(chebi_role_m, #[ !is.na(value) ],
                    cas + chebiid ~ variable,
                    value.var = 'value',
                    fun.aggregate = function(x) as_true(length(which(!is.na(x)))))
clean_names(chebi_role2)
setcolorder(chebi_role2, c('cas', 'chebiid'))
## chemical classes
chebi_class = copy(ont_par)
cols_class = c('acylamino.acid', 'amide', 'anilide', 'anilinopyrimidine', 
  'aromatic', 'aryl.phenyl.ketone', 
  'avermectin', 'benzamide', 'benzanilide', 'benzimidazole', 'benzimidazolylcarbamate', 
  'benzothiazole', 'benzoylurea', 'bisacylhydrazine', 'bridged.diphenyl', 
  'bridged.diphenyl', 'carbamate', 'carbanilate', 'cas', 
  'chebiid', 'chloropyridyl', 'conazole', 'cyclodiene.organochlorine', 
  'dicarboximide', 'dichlorophenyl.dicarboximide', 'dinitrophenol', 
  'formamidine', 'furamide', 'furanilide', 'imidazole', 'morpholine', 
  'nereistoxin.analogue', 'organochlorine', 'organofluorine', 
  'organophosphate', 'organosulfur', 'organothiophosphate', 'organotin', 
  'phenoxy', 'phenylsulfamide', 'phthalimide', 
  'pyrazole', 'pyrazole', 'pyrethroid.ester', 'pyrethroid.ether', 
  'pyrimidinamine', 'pyrimidine', 'quinoxaline', 'spinosyn', 'sulfite.ester',
  'sulfonamide', 'sulfonanilide', 'tetrazine', 'thiourea', 'triazine',
  'triazole', 'valinamide')
for (i in cols_class) {
  chebi_class[ grep(i, chebiName), (i) := TRUE ]
}
chebi_class_m = melt(chebi_class[ , .SD, .SDcols =! 'chebiName' ], id.vars = c('cas', 'chebiid'))
chebi_class2 = dcast(chebi_class_m, #[ !is.na(value) ],
                    cas + chebiid ~ variable,
                    value.var = 'value',
                    fun.aggregate = function(x) as_true(length(which(!is.na(x)))))
...
# write -------------------------------------------------------------------
# chebi general
write_tbl(chebi_prop, user = DBuser, host = DBhost, port = DBport, password = DBpassword,
          dbname = DBetox, schema = 'chebi', tbl = 'chebi_prop',
          key = 'cas',
          comment = 'Results from ChEBI (properties)')
# role
write_tbl(chebi_role2, user = DBuser, host = DBhost, port = DBport, password = DBpassword,
          dbname = DBetox, schema = 'chebi', tbl = 'chebi_role',
          key = 'cas',
          comment = 'Results from ChEBI (ontology - role)')
# class
write_tbl(chebi_class2, user = DBuser, host = DBhost, port = DBport, password = DBpassword,
          dbname = DBetox, schema = 'chebi', tbl = 'chebi_class',
          key = 'cas',
