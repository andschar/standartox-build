...
# Query EPA data
```{r query-epa, eval=TRUE, include = FALSE}
if (online) {
  todo = unique(psm$casnr) # remove duplicates
  todo = todo[which(nchar(todo) > 0)] # 495 substances!
...
## refine effect indices (could also be done in the query)
...
# Durations
```{r duration}
epa2[ , duration2 := ifelse(duration >= 21 & duration <= 27, 24,
                     ifelse(duration >= 46 & duration <= 50, 48,
                     duration))]
# epa2[ , .N, duration][N > 10][order(duration)] # debuging
```

# Cleaning
```{r cleaning}
rm(d, drv, con, epa1, miss, subst_check, psm)
```
